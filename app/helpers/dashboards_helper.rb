module DashboardsHelper
	...
	def todays_earning
		amount = []
		current_doctor.patients.each{|patient| amount << patient.prescriptions.where(created_at: Time.zone.now.beginning_of_day..Time.zone.now.end_of_day).pluck(:doctor_bill_amount).map(&:to_i).sum}
